 ...
 <body>
   ...
   ?>"><?php echo $row['answer4'] ?>
     ...
     if(isset($_POST['button'])){
       ...
       $selectQuizType = mysqli_query($connection, "SELECT * FROM quiz WHERE id = $quizId");
       $selectQuizType = $selectQuizType->fetch_assoc();
       $type = $selectQuizType['type'];
       $courseId = $selectQuizType['courseId'];

       $sqll = mysqli_query($connection, "INSERT INTO takequiz(userId, quizId, grade) VALUES ('$id', $quizId, $grade)");
       if($type == 1){
         $sql = mysqli_query($connection, "UPDATE points SET pointNr = pointNr + 1 WHERE userId = '$id' AND courseId = $courseId");
       }else if($type == 2){
         $sql = mysqli_query($connection, "UPDATE points SET pointNr = pointNr + 10 WHERE userId = '$id' AND courseId = $courseId");
       }
       header('location:viewQuiz.php?quiz='.$quizId);
     }
   }}
     ?>
 </ul>
