...
$approved = $result1->num_rows;
 ...
 <head>
   ...
   <link rel="stylesheet" href="css/professor_q.css">
 ...
 <body>
     ...
     <div id="MainScreen">
       ...
       <script>
       $('#New').click(function(){
           window.location.href='newQuizProf.php';
       })
       </script>
       ...
       <ul id="QuizList">
         ...
         if(isset($_POST['SubmitFilter'])){
           ...
           echo $courseFilter; echo $conceptFilter;
           if(!$courseFilter == "" && $conceptFilter == ""){
             $link = "location:myQuizzes.php?course=".$courseFilter;
             header($link);
           }else if(!$conceptFilter == "" && $courseFilter == ""){
             $link = "location:myQuizzes.php?concept=".$conceptFilter;
             header($link);
           }else if(!$conceptFilter == "" && !$courseFilter == ""){
             $link = "location:myQuizzes.php?concept=".$conceptFilter."&course=".$courseFilter;
             header($link);
           }
         }
         ...
         if(isset($_GET['concept']) && !isset($_GET['course'])){
           ...
           $sql = mysqli_query($connection, "SELECT DISTINCT Q.type AS type, Q.creatorId AS creatorId, U.first_name AS firstName, U.last_name AS lastName, C.name AS courseName,Q.id AS Qid, Q.name AS questionName, Q.description AS description FROM quiz Q INNER JOIN user U ON Q.creatorId = U.id INNER JOIN course C ON Q.courseId = C.id LEFT JOIN hasquestion H ON H.quizId = Q.id");
         }
     ...
     function openQuiz(courseId,userGrade){
       if(userGrade == 11){
         location.replace("completeQuizProf.php?quiz=" + courseId);
       }else{
         location.replace("viewQuizProf.php?quiz=" + courseId);
       }
     }
     </script>
     <script>
